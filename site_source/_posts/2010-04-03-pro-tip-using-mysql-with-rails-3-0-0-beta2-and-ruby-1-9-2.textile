...
I know "NoSQL":http://en.wikipedia.org/wiki/NoSQL is upon us, but some of you might *still* be stuck with MySQL for the time being. Saying that, the "mysql gem":http://rubygems.org/gems/mysql is not the best way to talk to your database using Ruby 1.9. It will simply ASCII-encode all the strings returned from the database, no matter what you set in database.yml. But fortunately, the solution is near! Just use the "ruby-mysql gem":http://rubygems.org/gems/ruby-mysql instead. It is made by the same guy, Tomita Masahiro, but targets the 1.9 branch of Ruby.
